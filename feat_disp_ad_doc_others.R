...
ctrnraw = fread("input/clicks_train.csv", select=c("display_id", "ad_id"))
ctstraw = fread("input/clicks_test.csv", select=c("display_id", "ad_id"))
craw = rbind(ctrnraw, ctstraw)
rm(ctrnraw, ctstraw); gc()
event = fread("input/events.csv", select=c("display_id", "document_id"))
...
craw = craw[,(paste0(paste("5:",document_ad_id,":1", sep=""), collapse = " ")), by=display_id]
setnames(craw, c("display_id", "document_ad_id"))
setkeyv(craw, "display_id")

cat("Write out the file")
write.csv(craw, gzfile("document_ad_id_others.csv.gz"), row.names = F)
gc()
