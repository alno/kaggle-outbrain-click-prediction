...
def fit_predict(train_file, pred_file):
    ...
    test['score'] = test['ad_id'].map(ad_pos_cnt).fillna(0) / (test['ad_id'].map(ad_cnt).fillna(0) + reg)
...
score = score_prediction(pred)
name = gen_prediction_name('ad-mean', score)

print "  Score: %.5f" % score

pred[['display_id', 'ad_id', 'score']].to_pickle('preds/%s.pickle' % name)
