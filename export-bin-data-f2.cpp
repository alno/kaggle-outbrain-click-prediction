...
void writer::write(const reference_data & data, const std::vector<std::vector<std::string>> & rows) {
    ...
    features.hashed(2, event.country + event.state);
    ...
    for (auto it = ev_doc_topics.first; it != ev_doc_topics.second; ++ it)
        features.hashed(8, it->second.first, it->second.second);
    /*
...
int main() {
    ...
    generate_files<reference_data, writer>(data, build_filesets(files, features, "_bin_f2"));
