...
print "Loading data..."
dts = pd.read_csv('uuid_srce_ids.csv.gz')
dts['uid'] = dts['uuid'].map(uids)
dts['source_id'] = dts['document_id'].map(convert_id_list)

print "Saving..."
dts[['uid', 'source_id']].to_csv('cache/viewed_doc_sources.csv.gz', index=False, compression='gzip')
