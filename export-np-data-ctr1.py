...
def export_data(clicks_file_name, out_name):
    ...
    res = np.memmap("cache/%s_np_ctr1.npy" % out_name, dtype='float32', mode='w+', shape=(n_rows, 18))
    ...
    zipped_stream = izip(
        click_stream, leak_stream,
        uid_viewed_ads_stream, uid_viewed_ad_srcs_stream, uid_viewed_ad_tops_stream, uid_viewed_ad_cats_stream,
        g2_viewed_ads_stream, g2_viewed_ad_srcs_stream, g2_viewed_ad_tops_stream, g2_viewed_ad_cats_stream
    )

    chunk_start = 0
    with tqdm(total=n_rows, desc='  Exporting %s' % clicks_file_name, unit='rows') as pbar:
        for clk, leak, uv_ad, uv_ad_src, uv_ad_top, uv_ad_cat, g2_ad, g2_ad_src, g2_ad_top, g2_ad_cat in zipped_stream:
            ...
            res[chunk_start:chunk_end, 14] = ctr_logit(g2_ad_top['top_past_views'], g2_ad_top['top_past_clicks'])
            res[chunk_start:chunk_end, 15] = ctr_logit(g2_ad_top['top_future_views'], g2_ad_top['top_future_clicks'])
